...
2. **Load** (`eval.ts:80-86`, then `lib/loaders.ts`). `loadSkillMd()` reads `SKILL.md`, validates frontmatter via `parseSkillMd` / `parseSkillMdEnterprise` (Zod). `loadSkillEvalSpec()` reads `eval-spec.yaml` and validates via `SkillEvalSpecSchema`. The skill profile defines which skill siblings exist (for trigger precision/recall), the test-case roster, and the criteria. Failure point: schema violation — thrown with concatenated Zod error messages, exit 1.
