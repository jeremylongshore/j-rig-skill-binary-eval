<!-- PROVENANCE: Copied from ~/000-projects/nixtla/000-docs/000a-skills-schema/SKILLS-STANDARD-COMPLETE.md on 2026-03-24.
     Reconciled against AgentSkills.io spec and /skill-creator source-of-truth.
     Any conflicts resolved in favor of official sources.
     CONFLICTS NOTED:
     - allowed-tools delimiter: This file documents comma-delimited CSV format
       throughout (e.g., "Read,Write,Glob,Grep,Edit"). AgentSkills.io spec
       requires space-delimited. Enterprise extension — does not contradict.
     - compatible-with field: Not defined in this spec. AgentSkills.io enterprise
       tier includes it as required. Enterprise additive gap.
     - metadata field: Correctly absent from frontmatter schema. AgentSkills.io
       allows it but /skill-creator lists it in INVALID_SKILL_FIELDS.
     - Folder name matching: Section 2 states "Folder names must match the name
       field exactly." AgentSkills.io does not enforce this at runtime but
       recommends it. Enterprise extension — does not contradict. -->
